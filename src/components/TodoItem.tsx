  ...
  const [isCompleted, setIsCompleted] = useState(todo.isCompleted || false);
	...
	if (isEditing) {
		return (
			...
			>
				<h2 className="text-xl font-semibold text-gray-900 mb-4">Edit Todo</h2>
				...
				<TodoFormFields
					...
					titleId={`edit-title-${todo.id}`}
					descriptionId={`edit-description-${todo.id}`}
					completedId={`edit-completed-${todo.id}`}
					autoFocus={true}
				/>
